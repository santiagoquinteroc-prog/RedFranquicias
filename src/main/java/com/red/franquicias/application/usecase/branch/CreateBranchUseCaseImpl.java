...
public class CreateBranchUseCaseImpl implements CreateBranchUseCase {
    ...
    public CreateBranchUseCaseImpl(BranchRepositoryPort branchRepositoryPort, FranchiseRepositoryPort franchiseRepositoryPort) {
    ...
    public Mono<Branch> create(Branch branch) {
        try {
            BranchValidator.validateName(branch.getName());
        ...
        return franchiseRepositoryPort.findById(branch.getFranchiseId())
                ...
                .flatMap(franchise -> branchRepositoryPort.existsByNameAndFranchiseId(branch.getName(), branch.getFranchiseId())
                        .flatMap(exists -> {
                            if (exists) {
